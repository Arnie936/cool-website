...
<div data-cw-mode="worldflight" data-cw-seam="0.12">
  ...
  <div data-cw-world>
    <div data-cw-segment data-cw-w="0.95" data-cw-linger="0.3"
         data-cw-waypoint="Surface">
      <img class="cw-world__poster" src="assets/p1.webp" alt="" decoding="async">
      <video data-cw-src="assets/leg1.mp4"
             data-cw-src-mobile="assets/leg1-m.mp4"></video>
    </div>
    <div data-cw-segment data-cw-w="0.9" data-cw-linger="0.42"
         data-cw-waypoint="Thermocline">
      <img class="cw-world__poster" src="assets/p2.webp" alt="" decoding="async">
      <video data-cw-src="assets/leg2.mp4"
             data-cw-src-mobile="assets/leg2-m.mp4"></video>
    </div>
  ...
  <div data-cw-world-copy>
    ...
    <div class="cw-copy cw-copy--lead" data-cw-copy data-cw-window="hero"> … </div>
    <div class="cw-copy cw-copy--trail" data-cw-copy data-cw-window="0.38 0.66"> … </div>
    <div class="cw-copy cw-copy--lead" data-cw-copy data-cw-window="finale"> … </div>
  </div>
...
`CoolWebsite.mount(document)` as usual. The mode composes with nothing else on
...
|---|---|---|---|
| `data-cw-mode="worldflight"` | mode root | n/a | Turns the page into one flight. |
| `data-cw-seam` | mode root | `0.12` | Crossfade band, in viewport-heights of scroll. Clamped 0.02 to 0.4. |
| `data-cw-world` | stage | n/a | The single fixed stage. Gets `.cw-world`. |
| `data-cw-segment` | leg | n/a | One leg. Holds a poster and a clip. |
| `data-cw-w` | leg | `1.3` | Scroll this leg owns, in viewport-heights. |
| `data-cw-linger` | leg | `0` | Dwell remap for this leg only. Clamped to 0.6. |
| `data-cw-waypoint` | leg | n/a | Label published on the waypoint event. |
| `data-cw-world-copy` | copy layer | n/a | Fixed overlay. Gets `.cw-world__copy`. |
| `data-cw-copy` | copy block | n/a | A windowed block of type. |
| `data-cw-window` | copy block | n/a | `hero` \| `finale` \| `from to [in [out]]`. |
| `data-cw-spacer` | spacer | n/a | The scroll track. Engine sets its height. |
| `data-cw-lerp` | root or `<video>` | `0.18` | Playhead smoothing. See devices.md. |
...
- `data-cw-window="hero"`: present from the first pixel, fades out by 0.62 of
...
- `data-cw-window="finale"`: fades in from 0.4 of the last leg and holds to the
  end.
- `data-cw-window="0.38 0.66"`: a plateau window across those track fractions:
...
`.cw-world__cwrim` is provided for a scrim div on the copy side. Shape it to
where the copy actually sits. The stock `.cw-scrim--band` tops out at 58% of the
frame, and footage that stays bright past that can fail the legibility check even
...
The engine publishes the current leg index as `--cw-seg` and its local progress
as `--cw-segp`, on the mode root and on `:root`, and fires a `sc:waypoint`
...
- Ship `data-cw-src-mobile` for every leg. The engine picks it on coarse
...
registered, the clips fetched and decoded, `cw-has-clip` is on the segments, and
...
fine and often correct; exceeding it without checking is not. Compare actual
video imagery at adjacent scroll positions in the integrated browser. At
0.216vh/s a 0.12vh gap maps to about half a second of film; verify the image
really advances rather than trusting the mapping alone.

**Damp the playhead and widen the joins.** `data-cw-lerp` defaults to 0.18;
...
removes wheel-event judder. Widen `data-cw-seam` from 0.12 to ~0.16 for the same
reason: a longer crossfade band gives each join more room to disappear in.

Changing weights moves every leg boundary, so **every `data-cw-window` has to be
...
| **One accent, one scrim shape, copy anchored off the bright centre.** | Check copy blocks at their least readable visible frames in the integrated browser; report numerical contrast only when actually measured. |

---

## 9. Verifying

Use the integrated browser procedure in [verify.md](verify.md). Inspect each
leg at several positions and sample more closely before, within, and after
every seam. Let the displayed frame settle before capturing screenshots.

Check the worldflight-specific behavior:

- The stage stays fixed and the spacer provides the intended scroll distance.
- Forward and backward scrolling move the film smoothly toward its target,
  without reversing or overshooting; the image settles when scrolling stops.
- Each incoming leg reaches full opacity. The outgoing leg remains opaque
  until covered, so the page ground never flashes through the dissolve.
- Every clip displays changing decoded frames instead of remaining on a poster.
- Copy stays within its intended transform cap and is readable through seams.
- No dead spans remain except documented holds; the final frame resolves.
- Under reduced motion, posters, copy, navigation, and all controls remain
  understandable and reachable, with no video fetch when that can be checked.

Use only diagnostic capabilities exposed by the integrated browser. Separate
visual observations from measured values and state unsupported checks plainly.
