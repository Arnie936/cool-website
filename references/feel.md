...
| The most scroll room | The largest `data-cw-span` on the page, and the `data-cw-dwell` that makes the camera settle exactly on it |
...
- **Pointer parallax that moves the world, not a card.** `data-cw-spotlight`
  publishes `--cw-mx` / `--cw-my`. Drive a background layer's transform off them
...
- **Dwell-triggered detail.** Hold still on an act and something further arrives:
  ...
  `--cw-p` staying constant across a few frames, in the page's own JS, and reveal
...
| A long pin | Held breath, pressure, attention | `data-cw-span` 3+, overlapping cues, one idea |
| An empty viewport before a reveal | Silence before the drop | A ground-only act, no cue until the next one |
| A slow settle mid-act | The shot landing | `data-cw-dwell` 0.35 to 0.6 with the cue peak on the settle |
| A fast cue with a long plateau | Confidence, arrival | `data-cw-cue="0.1 0.9 0.08 0.4"` |
...
load, and both can look like dead scroll during review. If you are using the empty
...
A verification pass after the functional checks in SKILL.md Step 5, using
integrated-browser screenshots and a live scroll. Those checks establish whether
the page works.
