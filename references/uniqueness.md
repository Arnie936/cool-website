...
`data-cw-tilt="6"` on each card, and close on `data-cw-act="pin"` with
`data-cw-span="1.15"`, `data-cw-spotlight` on the stage and
`data-cw-magnet="0.26"` on the CTA. All four land between 13.6 and 13.8
...
telemetry, pointer devices where the real product would have them, and `--cw-p`
...
> **This grammar REQUIRES worldflight mode.** `data-cw-mode="worldflight"`, one
...
**Forbids:** section boundaries of any kind. No `cw-section` blocks, no acts at
...
**Leans on:** worldflight legs with `data-cw-linger`, copy windows against the
...
be right), `pin` with scale driven from `--cw-p`, `reveal` as a wipe across
...
**Leans on:** `pin` with divider position driven from `--cw-p`, `reveal` per
...
**Forbids:** any act over ~1.4 viewport-heights; `data-cw-dwell` above 0.1;
...
from page scroll rather than from an act's `--cw-p`, since the whole point is
...
Coded in the page, with `data-cw-*` attributes of your own naming or plain
inline JS reading `--cw-p`. The engine stays untouched, always.
...
- **A line drawing that builds itself.** An SVG technical illustration whose
  `stroke-dashoffset` is driven from `--cw-p`, so scrolling literally draws the
...
- `data-cw-tilt="9"` instead of `6`. Any parameter change to any kit device.
