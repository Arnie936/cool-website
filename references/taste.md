...
- Use the 4px-base scale (`--cw-1` … `--cw-11`). A 4-base gives the useful
...
- Section padding is fluid (`--cw-section`). A phone should not inherit desktop
...
- Gutters scale with viewport (`--cw-gutter`). Full-bleed media goes edge to
...
- **Tracking tightens as size grows.** A face set at 6rem with default tracking
  reads loose and amateur. The ramp handles this: `--cw-track-tight` on display,
  `--cw-track-normal` on body. This is optical correction, not decoration.
- **Body measure 45 to 75ch.** `--cw-measure` is 62ch. A full-width paragraph on
...
- **Step the hero down one rung below ~700px.** `--cw-t-4xl` floors at 3.4rem,
  which is a *desktop* floor: at 390px it wraps a normal hero headline to six
  lines. `--cw-t-2xl` on the hero inside a phone media query fixes it. The
...
**Redefining `--cw-ink` on a subtree does not re-ink the text under it.**
...
silently: an inverted section can render bone type on concrete with poor
contrast. Check the actual foreground and background colors in that section. The fix is one declaration on the same subtree:

```css
.section--light { --cw-ink: #14110C; --cw-ink-soft: #4A443A; color: var(--cw-ink); }
```
...
1. **A corner** of density, sized to the copy block. `.cw-scrim--lead` /
   `.cw-scrim--trail`. Right when the copy is anchored to a corner on a wide
...
2. **A band**, `.cw-scrim--band`, transparent above roughly 58%. Right whenever
   the copy spans the full width of the frame, which is what *both* corner
   anchors become below 860px. The engine already switches `.cw-scrim--trail` to
...
**Keep a scrim separate from the text it protects.** If an available contrast
measurement hides copy to inspect the background, it must retain the scrim.
A sibling element avoids hiding a `::before` scrim along with the text.
See verify.md for the distinction between visual and measured checks.
...
clip brightens under the copy. Check the changing background in the integrated
browser; report a ratio only when it has actually been measured.
...
2. **Edge light.** A 1px top highlight (`--cw-edge`) sells a raised surface
...
5. **Grain.** A flat dark ground bands on real displays. `.cw-grain` at 4-5%
   opacity is the difference between "a dark page" and "a lit room".

Three elevation steps (`--cw-e1/2/3`) and no more. If everything is elevated,
nothing is.
...
- Built-in CSS easings are too weak. Use `--cw-ease-out`
...
gets skipped most reliably. `cool-website.css` themes all of these; if you fork
it, keep them:
