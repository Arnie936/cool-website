...
3. **`--cw-edge` is an ink-tinted inset highlight** tuned for a near-black
   ground. On paper it reads as dirt along the top lip. Invert it to white, or
   drop it and carry the raise with a hairline.
4. **The `--cw-e1/2/3` shadow alphas are tuned for a near-black ground** too.
   Halve them and re-tint `--cw-shadow-color` toward the canvas hue.

The contrast direction flips with all of this: dark type fails on the *darkest*
patch under it, not the brightest. Check each line against the appropriate
background extreme in the integrated browser (see [verify.md](verify.md)).
