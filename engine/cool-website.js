/* ============================================================================
   cool-website: a scroll-driven interaction runtime
   ----------------------------------------------------------------------------
   ...
   it up with data-cw-* attributes; the engine reads them and drives them from a
   ...
   ---------------------------------------------------------------------------
     <section data-cw-act="pin" data-cw-span="2.5"> ... </section>

     data-cw-act   scrub | pin | pan | flow   (default: flow)
     data-cw-clip-map="travel" opts a pinned act OUT of full-life clip mapping.
                   ...
                   page moves. Pair it with data-cw-dwell, which moves quickly at
                   the edges and settles in the middle, so the fast motion lands
                   on the two slides and the settle lands inside the pin where
                   the copy is. data-cw-runout is accepted and ignored; it named
                   the old opt-in for the exit half of this.
     data-cw-span  viewport-heights of scroll this act owns. Pinned devices only
                   (scrub/pin/pan). Default 1.5. The engine sets the outer
                   height and sticks the first .cw-stage / [data-cw-stage] child.

     Every act exposes a normalized progress p (0..1):
       pinned  p = (y - top) / (height - vh)
       flow    p = (y + vh - top) / (height + vh)
     and publishes it as --cw-p on the act element, so CSS can read it too.

   ---------------------------------------------------------------------------
   DEVICES: what p drives
   ---------------------------------------------------------------------------
     data-cw-scrub            on <video>. p scrubs currentTime. Blob-loaded, so
                              it seeks without needing HTTP range support.
     data-cw-sequence="a/{i}.webp:120:1"
                              on <canvas>. p scrubs an image sequence
                              (path template : frameCount : startIndex).
     data-cw-pan="0.6"        on a wide rail inside data-cw-act="pan". p drives
                              horizontal travel. Value = extra travel multiplier.
     data-cw-parallax="-0.2"  translateY by rate * act-progress * viewport.
                              Negative = moves up faster than scroll (recedes).
     data-cw-cue="0.1 0.5"    opacity/rise keyed to p. One value = enter+hold.
                              Two = enter..leave. Add a third for the hold point.
     data-cw-kinetic="lines"  lines | words | chars. Splits the element and
                              staggers its reveal across the cue window.
     data-cw-reveal="up"      up | down | left | right | iris. clip-path wipe.
     data-cw-count="0 4200"   number bloom across the cue window. Outside any
                              act it ticks up once on entry instead, over
                              data-cw-count-ms (default 1400).
     data-cw-in               flow-section reveal, fires once on entry via
                              IntersectionObserver (cheaper, and content that
                              re-hides on scroll-up is a defect, not an effect).
                              data-cw-stagger="60" on the parent staggers kids.
     data-cw-drift="#07090c"  page background interpolates toward this colour
                              while the act is on screen.

   ---------------------------------------------------------------------------
   WORLDFLIGHT: the page mode for one continuous flight
   ---------------------------------------------------------------------------
     <div data-cw-mode="worldflight" data-cw-seam="0.12">
       <div data-cw-world>
         <div data-cw-segment data-cw-w="1.4" data-cw-linger="0.3"
              data-cw-waypoint="Approach">
           <img class="cw-world__poster" src="p1.webp" alt="">
           <video data-cw-src="leg1.mp4" data-cw-src-mobile="leg1-m.mp4"></video>
         </div>
         ... more segments, in flight order ...
       </div>
       <div data-cw-world-copy>
         <div class="cw-world__cwrim cw-scrim cw-scrim--band"></div>
         <div data-cw-copy data-cw-window="hero"> ... </div>
         <div data-cw-copy data-cw-window="0.34 0.56"> ... </div>
         <div data-cw-copy data-cw-window="finale"> ... </div>
       </div>
       <div data-cw-spacer aria-hidden="true"></div>
     </div>
   ...
   the film timeline and the overlay opacity. Nothing else moves.
     ...
     data-cw-w        viewport-heights this segment owns. Default 1.3.
     data-cw-linger   dwell remap for this leg only (see lingerEase). Max 0.6.
     data-cw-seam     crossfade band, in viewport-heights of scroll. Default 0.12.
     data-cw-waypoint label published on --cw-seg / the sc:waypoint event, so a
                      page can draw its own route rail. The engine draws none.
     data-cw-window   on a copy block: "hero" | "finale" | "from to [in [out]]"
   ...
   ---------------------------------------------------------------------------
     data-cw-tilt="8"         3D tilt toward the pointer, spring-damped, degrees.
     data-cw-magnet="0.35"    element drifts toward the pointer inside its bounds.
     data-cw-spotlight        publishes --cw-mx/--cw-my (0..1) for a light that
   ...
   current time toward it at a fixed fraction per frame (data-cw-lerp, default
...
(function (global) {
  ...
  function lerpRate(el) {
    var v = parseFloat(el && el.getAttribute && el.getAttribute('data-cw-lerp'));
  ...
  function splitText(el, mode) {
    if (el.__cwSplit) return el.__cwSplit;
    ...
    if (mode === 'chars' || mode === 'words') {
      ...
      parts.forEach(function (t) {
        ...
        mask.className = 'cw-split';
        var inner = document.createElement('span');
        inner.className = 'cw-split__i';
        inner.textContent = t;
      ...
      lines.forEach(function (words, li) {
        var mask = document.createElement('span');
        mask.className = 'cw-split cw-split--line';
        var inner = document.createElement('span');
        inner.className = 'cw-split__i';
  ...
  function mount(root, opts) {
    ...
    var progressBar = root.querySelector('[data-cw-progress]');
    ...
    // ---- collect acts -----------------------------------------------------
    Array.prototype.forEach.call(root.querySelectorAll('[data-cw-act]'), function (el) {
      var device = el.getAttribute('data-cw-act') || 'flow';
      var pinned = device === 'scrub' || device === 'pin' || device === 'pan';
      var act = {
        el: el,
        device: device,
        pinned: pinned,
        span: parseFloat(el.getAttribute('data-cw-span')) || (pinned ? 1.5 : 0),
        dwell: parseFloat(el.getAttribute('data-cw-dwell')) || 0,
        clipTravel: pinned && el.getAttribute('data-cw-clip-map') === 'travel',
      ...
      if (pinned) {
        act.stage = el.querySelector('[data-cw-stage]') || el.querySelector('.cw-stage');
        if (act.stage) act.stage.classList.add('cw-stage');
        el.classList.add('cw-act--pinned');
      }

      // scrub video
      var v = el.querySelector('video[data-cw-scrub]');
      if (v) act.video = makeClip(v, el);

      // image sequence
      var cv = el.querySelector('canvas[data-cw-sequence]');
      if (cv) {
        var spec = cv.getAttribute('data-cw-sequence').split(':');
        act.seq = {
      ...
      // horizontal rail
      act.rail = el.querySelector('[data-cw-pan]');
      if (act.rail) act.railExtra = parseFloat(act.rail.getAttribute('data-cw-pan')) || 0;

      // cues
      Array.prototype.forEach.call(el.querySelectorAll('[data-cw-cue]'), function (c) {
        var nums = (c.getAttribute('data-cw-cue') || '').trim().split(/\s+/).map(parseFloat);
        ...
        var cue = {
          ...
          rise: parseFloat(c.getAttribute('data-cw-rise')),
          kinetic: c.getAttribute('data-cw-kinetic'),
          units: null, state: -1
        };
      ...
      // parallax
      Array.prototype.forEach.call(el.querySelectorAll('[data-cw-parallax]'), function (c) {
        act.parallax.push({ el: c, rate: parseFloat(c.getAttribute('data-cw-parallax')) || 0 });
      });

      // reveals
      Array.prototype.forEach.call(el.querySelectorAll('[data-cw-reveal]'), function (c) {
        var nums = (c.getAttribute('data-cw-reveal-at') || '0 0.5').trim().split(/\s+/).map(parseFloat);
        act.reveals.push({ el: c, dir: c.getAttribute('data-cw-reveal') || 'up', from: nums[0] || 0, to: nums[1] || 0.5 });
      });

      // counters
      Array.prototype.forEach.call(el.querySelectorAll('[data-cw-count]'), function (c) {
        var nums = (c.getAttribute('data-cw-count') || '').trim().split(/\s+/);
        var at = (c.getAttribute('data-cw-count-at') || '0.1 0.55').trim().split(/\s+/).map(parseFloat);
      ...
      var d = el.getAttribute('data-cw-drift');
      if (d) { var rgb = parseColor(d); if (rgb) drifts.push({ act: act, rgb: rgb }); }
    });

    // ---- collect worldflights --------------------------------------------
    Array.prototype.forEach.call(root.querySelectorAll('[data-cw-mode="worldflight"]'), function (el) {
      var W = {
        el: el,
        stage: el.querySelector('[data-cw-world]') || el.querySelector('.cw-world'),
        copyLayer: el.querySelector('[data-cw-world-copy]') || el.querySelector('.cw-world__copy'),
        spacer: el.querySelector('[data-cw-spacer]') || el.querySelector('.cw-world__spacer'),
        seam: 0, segs: [], copies: [], total: 0, top: 0, index: -1, checked: false
      };
      var seam = parseFloat(el.getAttribute('data-cw-seam'));
      ...
      if (W.stage) W.stage.classList.add('cw-world');
      if (W.copyLayer) W.copyLayer.classList.add('cw-world__copy');
      if (W.spacer) W.spacer.classList.add('cw-world__spacer');

      Array.prototype.forEach.call(el.querySelectorAll('[data-cw-segment]'), function (s) {
        var seg = {
          el: s,
          w: parseFloat(s.getAttribute('data-cw-w')) || 1.3,
          linger: clamp(parseFloat(s.getAttribute('data-cw-linger')) || 0, 0, 0.6),
          label: s.getAttribute('data-cw-waypoint') || '',
          poster: s.querySelector('[data-cw-poster]') || s.querySelector('.cw-world__poster') || s.querySelector('img'),
          c0: 0, c1: 0, local: 0, op: -1, z: -1, clip: null
        };
        s.classList.add('cw-world__seg');
        if (seg.poster) seg.poster.classList.add('cw-world__poster');
        var v = s.querySelector('video');
        if (v) {
          // Mark it as a scrub clip whether or not the author did. Everything
          // downstream, the stylesheet and diagnostic consumers included,
          // finds scrub media by this attribute.
          v.setAttribute('data-cw-scrub', '');
      ...
      Array.prototype.forEach.call(el.querySelectorAll('[data-cw-copy]'), function (cEl) {
        var spec = (cEl.getAttribute('data-cw-window') || '').trim();
    ...
    if ('IntersectionObserver' in window) {
      io = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) {
          ...
          var stagger = parseFloat(el.getAttribute('data-cw-stagger'));
          if (!isNaN(stagger)) {
            Array.prototype.forEach.call(el.children, function (kid, i) {
              kid.style.transitionDelay = (i * stagger) + 'ms';
              kid.classList.add('cw-in');
            });
          }
          io.unobserve(el);
        });
      }, { rootMargin: '0px 0px -12% 0px', threshold: 0.01 });
      Array.prototype.forEach.call(root.querySelectorAll('[data-cw-in]'), function (el) { io.observe(el); });
    } else {
      Array.prototype.forEach.call(root.querySelectorAll('[data-cw-in]'), function (el) { el.classList.add('cw-in'); });
    }

    // ---- entry counters (fire once) --------------------------------------
    // A [data-cw-count] outside any pinned act is not scrubbed by scroll; it
    // ticks up once when it enters view, over data-cw-count-ms (default 1400).
    ...
    (function () {
      ...
      var num = function (s) { return parseFloat(String(s).replace(/,/g, '')) || 0; };
      var spec = function (c) { return (c.getAttribute('data-cw-count') || '').trim().split(/\s+/); };
      var els = Array.prototype.filter.call(root.querySelectorAll('[data-cw-count]'), function (c) {
        return !c.closest('[data-cw-act]');
      });
      if (!els.length) return;
      function run(c) {
        var nums = spec(c);
        var a = num(nums[0]), b = num(nums[1]), tpl = nums[1] || '0';
        var ms = parseFloat(c.getAttribute('data-cw-count-ms')) || 1400;
    ...
    function layout() {
      ...
      // Set in pixels, not vh, because .cw-world is sized in svh on phones and a
      ...
      acts.forEach(function (a) {
        ...
        if (pos !== 'sticky' && pos !== '-webkit-sticky') {
          console.warn('[cool-website] act "' + (a.el.id || a.device) + '" will not pin: its stage computes ' +
            'position:' + pos + ', not sticky. Something is overriding .cw-stage.', a.stage);
        }
      });
      ...
      worlds.forEach(function (W) {
        ...
        W.checked = true;
        if (!W.stage) {
          console.warn('[cool-website] worldflight has no [data-cw-world] stage; nothing will fly.', W.el);
          return;
        }
        if (!W.spacer) {
          console.warn('[cool-website] worldflight has no [data-cw-spacer]; the page has no scroll track.', W.el);
        }
        ...
        if (wp !== 'fixed') {
          console.warn('[cool-website] worldflight stage computes position:' + wp + ', not fixed. ' +
            'Something is overriding .cw-world, and the flight will scroll off screen.', W.stage);
        }
      });
    ...
    function loadClip(V) {
      ...
      var src = V.el.getAttribute('data-cw-src') ||
                (isMobile() && V.el.getAttribute('data-cw-src-mobile')) ||
                V.el.currentSrc || V.el.src;
      if (isMobile() && V.el.getAttribute('data-cw-src-mobile')) src = V.el.getAttribute('data-cw-src-mobile');
      if (!src) return;
      V.loading = true;
      fetch(src).then(function (r) { if (!r.ok) throw new Error(r.status); return r.blob(); })
        .then(function (blob) {
          ...
          var reveal = function () {
            if (V.painted) return;
            V.painted = true;
            V.host.classList.add('cw-has-clip');
            V.el.classList.add('cw-has-clip');
          };
    ...
    function readWorld(W) {
      ...
      var cur = W.segs[k];
      W.el.style.setProperty('--cw-seg', String(k));
      W.el.style.setProperty('--cw-segp', cur.local.toFixed(4));
      docEl.style.setProperty('--cw-seg', String(k));
      docEl.style.setProperty('--cw-segp', cur.local.toFixed(4));
      if (k !== W.index) {
        W.index = k;
        try {
    ...
    function read() {
      ...
      for (var i = 0; i < acts.length; i++) {
        ...
        a.el.style.setProperty('--cw-p', a.p.toFixed(4));
      ...
      if (driftA) {
        docEl.style.setProperty('--cw-canvas', mixColor(driftB.rgb, driftA.rgb, driftT));
      }
    ...
    function initPointer() {
      ...
      Array.prototype.forEach.call(root.querySelectorAll('[data-cw-tilt]'), function (el) {
        tilts.push({ el: el, max: parseFloat(el.getAttribute('data-cw-tilt')) || 6, x: 0, ty: 0, tx: 0, y: 0 });
      });
      Array.prototype.forEach.call(root.querySelectorAll('[data-cw-magnet]'), function (el) {
        magnets.push({ el: el, k: parseFloat(el.getAttribute('data-cw-magnet')) || 0.3, x: 0, y: 0, tx: 0, ty: 0 });
      });
      Array.prototype.forEach.call(root.querySelectorAll('[data-cw-spotlight]'), function (el) {
        spots.push(el);
      });
      ...
      addEventListener('pointermove', function (e) {
        ...
        for (var s = 0; s < spots.length; s++) {
          ...
          spots[s].style.setProperty('--cw-mx', clamp01((e.clientX - sr.left) / sr.width).toFixed(3));
          spots[s].style.setProperty('--cw-my', clamp01((e.clientY - sr.top) / sr.height).toFixed(3));
        }
      }, { passive: true });
    ...
    // definition. behavior:'instant' on purpose: cool-website.css sets
    ...
    addEventListener('focusin', function (e) {
      ...
      var act = el.closest('[data-cw-act]');
      if (!act || !root.contains(act)) return;
      var cue = el.closest('[data-cw-cue]');
      if (!cue) return;
    ...
    if (document.fonts && document.fonts.ready) {
      ...
      document.fonts.ready.then(function () {
        acts.forEach(function (a) { a.cues.forEach(function (q) { if (q.kinetic && q.units) { q.el.__cwSplit = null; q.units = null; } }); });
        layout();
      });
    }
    ...
    global.CoolWebsite.instances.push(api);
    return api;
  }

  // The instance list is not an API for the page. It is how the verification
  // inspector can read the real playhead records: a screenshot taken mid-lerp is a
  // frame the page never actually holds, so an inspector has to be able to ask
  // whether the playhead has arrived rather than guess with a timeout.
  global.CoolWebsite = { mount: mount, reduce: reduce, instances: [] };
})(window);
