#!/usr/bin/env bash
# cool-website: encode a clip for scrubbing, not for playback.
#
...
#   COOL_WEBSITE_CRF=23 ./encode.sh in.mp4 out.mp4    or the env var
#
...
pick_ffmpeg() {
  ...
  echo "ERROR: no full ffmpeg build found. Set COOL_WEBSITE_FFMPEG to one." >&2
  return 1
}
FFMPEG="$(pick_ffmpeg)"
